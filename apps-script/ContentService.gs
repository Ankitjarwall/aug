...
function buildContent_() {
  ...
  var hiddenSamples = {
    heroes: heroRows.filter(function(row) { return sampleOwned_(row.hero_id); }).map(function(row) { return row.hero_id; }),
    profiles: allProfileRows.filter(function(row) { return sampleOwned_(row.profile_id); }).map(function(row) { return row.profile_id; })
  };
  heroRows = heroRows.filter(function(row) { return !sampleOwned_(row.hero_id); });
  allProfileRows = allProfileRows.filter(function(row) { return !sampleOwned_(row.profile_id); });
  profileLinks = profileLinks.filter(function(row) { return !sampleOwned_(row.profile_category_id) && !sampleOwned_(row.profile_id); });
  if (hiddenSamples.heroes.length || hiddenSamples.profiles.length) debugWarn_("content.samples.hidden", "Generated sample profiles and heroes were excluded from production output.", hiddenSamples);
...
function enabled_(row) { return truthy_(row.enabled); } function sampleOwned_(value) { return String(value || "").indexOf("sample-") === 0; } function sortOrder_(a, b) { return num_(a.sort_order) - num_(b.sort_order); }
